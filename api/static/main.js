...
$(document).ready(() => {
  ...
  $('#view-button').on('click', (e) => {
    ...
    if (userTxt.value.length === 0) {
      ...
      // Check if the user exists by calling the API and checking 
...
function userSelected(userId) {
  // Store the userId in sessionStorage and change the window.location
  // to "user.html"
...
function movieSelected(imdb_id, movie_id, genres) {
  // Add the movie_id from database (not the imdb id)
...
function ratings() {
  ...
  // Make an API call to save user_id movie_id and the rating
...
function populatePriority3() {
  // Instead of the hardcoded value, fetch the recommendations from 
  ...
  axios.get('http://127.0.0.1:5000/get_popular_movies?uid=' + userId)
    .then((response) => {
      ...
      if (myData.hasMovies) {
        ...
        // Add a blue messages stating "Popular movies not found"
        let w = '';
        w = `
